...
const API_REQUEST_URL = "https://api.dify.ai/v1/chat-messages";
const API_KEY = "app-3LyQS6LCGJ7QNkpuVfdmaWnm"; // Token do seu app Dify
...
const requestApiResponse = async (incomingMessageElement) => {
    ...
    try {
      ...
      if (!data.answer) throw new Error("Resposta inválida ou vazia.");
  
      showTypingEffect(data.answer, marked.parse(data.answer), messageTextElement, incomingMessageElement);
...
suggestionItems.forEach(suggestion => {
    suggestion.addEventListener('click', () => {
        ...
        inputField.dispatchEvent(new Event("input")); // Garante ativação do botão

        // Atualiza a mensagem e envia
